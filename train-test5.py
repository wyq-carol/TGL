...
class GNNTimer2:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        # print(f"Time for this sample: {elapsed_time:.6f} seconds")
...
def prefetch_batch(grouped_data, prefetch_queue):
    for idx_prefetch_batch, rows in grouped_data:
        root_nodes = np.concatenate([rows.src.values, rows.dst.values, neg_link_sampler.sample(len(rows))]).astype(np.int32)
        ts = np.concatenate([rows.time.values, rows.time.values, rows.time.values]).astype(np.float32)
        if sampler is not None:
            if 'no_neg' in sample_param and sample_param['no_neg']:
            ...
            ret = sampler.get_ret()
            # time_sample += ret[0].sample_time()
            # TODO trade off transfer data cost and gpu compute cost
            mfgs = to_dgl_blocks(ret, sample_param['history'])
...
if __name__ == '__main__':
    ...
    for e in range(train_param['epoch']):
        ...
        grouped_data = df[:train_edge_end].groupby(group_indexes[0])
        ...
        last_root_nodes = None
        last_ts = None
        last_mem_edge_feats = None
        ...
        for idx, rows in grouped_data:
            ...
            with record_function("sample DGLBlock"):
                with GNNTimer(): # 0.001708
                    ...
                    # 相当于晚一个batch更新 期望并行的是当前batch的prefetch和上个batch的memory update # 这个能不能并行
                    if idx > 0 and idx < len(grouped_data) - 1:
                        block = None
                        
                        # print("start updating mailbox and memory")
                        # 它们三个能不能fusion一下
                        with GNNTimer2():
                            mailbox.update_mailbox(model.memory_updater.last_updated_nid, model.memory_updater.last_updated_memory, last_root_nodes, last_ts, last_mem_edge_feats, block)
                        with GNNTimer2():
                            mailbox.update_memory(model.memory_updater.last_updated_nid, model.memory_updater.last_updated_memory, last_root_nodes, model.memory_updater.last_updated_ts)
            ...
            with record_function("update memory"):
                with GNNTimer():
                    eid = rows['Unnamed: 0'].values
                    last_mem_edge_feats = edge_feats[eid] if edge_feats is not None else None
                time_prep += time.time() - t_prep_s # memory module相关的时间
                time_tot += time.time() - t_tot_s # 每轮epoch的时间
            print()
            print()

...
__global__ void fused_forward_kernel_small_f_sm(
  ...
  float *attn_val_sh = &edge_val_sh[shm_length * h];
...
void tgn_forward(int m, int nnz, int h, int f, const float *attn_row,
...
tgn_forward_cuda(torch::Tensor attn_row, torch::Tensor attn_col,
  ...
  tgn_forward(m, nnz, h, f, attn_row.data_ptr<float>(),
...
void tgn_backward(int m, int nnz, int h, int f, float negative_slope,
...
std::vector<torch::Tensor> tgn_backward_cuda(
  ...
  tgn_backward(m, nnz, h, f, negative_slope, row_ptr.data_ptr<int>(),

...
// TODO@mkj
std::vector<torch::Tensor> tgn_forward_cuda(
...
std::vector<torch::Tensor> tgn_forward(
    torch::Tensor attn_row,
    torch::Tensor attn_col,
    torch::Tensor row_ptr,
    torch::Tensor col_ind,
    ...
    torch::Tensor in_feat)
{
    ...
    return tgn_forward_cuda(attn_row, attn_col, row_ptr, col_ind, negative_slope, in_feat);
}

std::vector<torch::Tensor> tgn_backward_cuda(
    float negative_slope,
    torch::Tensor row_ptr,
    torch::Tensor col_ind,
...
std::vector<torch::Tensor> tgn_backward(
    float negative_slope,
    torch::Tensor row_ptr,
    torch::Tensor col_ind,
    torch::Tensor col_ptr,
    ...
    torch::Tensor grad)
{
    ...
    return tgn_backward_cuda(negative_slope, row_ptr, col_ind, col_ptr, row_ind, edge_max, edge_sum, in_feat, attn_row, attn_col, grad);
}

PYBIND11_MODULE(fused_tgn_SM, m)
{
    m.doc() = "fuse sparse ops in tgn into one kernel. ";
    m.def("tgn_forward", &tgn_forward, "fused tgn forward op");
    m.def("tgn_backward", &tgn_backward, "fused tgn backward op");
}
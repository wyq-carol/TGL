...
if __name__ == '__main__':
    ...
    #print(f'df={df}')
    #print(f'g.keys()={list(g.keys())}')
    # CSR  indptr=row  indices=col
    #print(f"g['indptr'] size={len(g['indptr'])}")
    #print(f"g['indptr']={g['indptr']}")
    #print(f"g['indices'] size={len(g['indices'])}")
    #print(f"g['indices']={g['indices']}")
    #print(f"g['ts'] size={len(g['ts'])}")
    #print(f"g['eid'] size={len(g['eid'])}")
    #print(f'node_feats={node_feats}')
    #print(f'edge_feats size={edge_feats.size()}')
    #print(f'train_edge_end={train_edge_end}, val_edge_end={val_edge_end}')
    ...
    if 'combine_neighs' in train_param and train_param['combine_neighs']:
        combine_first = True
    model = GeneralModel_fusion(gnn_dim_node, gnn_dim_edge, sample_param, memory_param, gnn_param, train_param, combined=combine_first).cuda()
    ...
    for e in range(train_param['epoch']):
        ...
        for batch_idx, rows in df[:train_edge_end].groupby(group_indexes[0]):
            # train_epoch()
            with GNNTimer():
            # sample 得到 DGLBLOCK
                t_tot_s = time.time()
            ...
            # prep mfgs
                if gnn_param['arch'] != 'identity':
                    # mfgs = to_dgl_blocks(ret, sample_param['history'])
                    node_feats, col_ptr, row_ind, num_nodes, num_edges, mfgs = to_dgl_blocks_our(
            ...
            with GNNTimer():
                if mailbox is not None:
                    ...
                    if memory_param['deliver_to'] == 'neighbors':
                        block = to_dgl_blocks(ret, sample_param['history'], reverse=True)[0][0]
                    # print(model.memory_updater.last_updated_nid.shape)
                    # print(model.memory_updater.last_updated_memory.shape)
                    # print(root_nodes.shape)
                    # print(ts.shape)
                    # print(mem_edge_feats.shape)
                    # print(block.shape)
                    # print(model.memory_updater.last_updated_ts.shape)
                    # exit()
                    # breakpoint()
